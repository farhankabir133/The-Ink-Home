...
async function main() {
  ...
  if (!src) {
    console.error('Usage: node scripts/copy-hero-frames.mjs /absolute/path/to/source-folder');
    process.exit(2);
  }

  const absSrc = path.resolve(src);
  const target = path.resolve(process.cwd(), 'public', 'hero-frames');
  ...
  // Sort files naturally by name to preserve order, then copy and rename to ezgif-frame-###.jpg
  files.sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));

  for (let i = 0; i < files.length; i++) {
    const srcFile = path.join(absSrc, files[i]);
    const destName = `ezgif-frame-${String(i + 1).padStart(3, '0')}${path.extname(files[i]).toLowerCase()}`;
  ...
  console.log('You can now run `npm run dev` and the hero will use these frames.');
}
